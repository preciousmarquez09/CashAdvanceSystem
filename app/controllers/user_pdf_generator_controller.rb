class UserPdfGeneratorController < ApplicationController
  ...
  def user
    ...
    send_data pdf.render, filename: 'users_list.pdf', type: 'application/pdf', disposition: params[:mode] === 'download' ? 'attachment' : 'inline'
  end

  def audit_logs
    @auditlogs = AuditLog.includes(:user).order(created_at: :desc)
    pdf = Prawn::Document.new
    ...
    data = [["Employee ID", "Name", "Action", "Date"]]
  
    @auditlogs.each do |auditlog|
      full_name = auditlog.user.present? ? "#{auditlog.user.f_name.capitalize} #{auditlog.user.l_name.capitalize}" : "N/A"
      data << [
        auditlog.user&.employee_id || "N/A",
        full_name,
        auditlog.action&.capitalize || "N/A",
        auditlog.created_at&.strftime('%B %-d, %Y') || "N/A"
      ]
    end
  
    pdf.table(data, width: pdf.bounds.width, header: true, cell_style: { borders: [:bottom, :top, :left, :right],
       padding: [5, 10], size: 10,  border_color: '000000', align: :center, overflow: :shrink_to_fit }) do |t|
    ...
    send_data pdf.render, filename: 'audit_logs.pdf', type: 'application/pdf', disposition: params[:mode] == 'download' ? 'attachment' : 'inline'
     
  end
  
  def download_complete
    if params[:message] == "cashadv"
      flash[:notice] = "Cash Advance PDF successfully saved!"
    elsif params[:message] == "user"
      flash[:notice] = "Users List PDF successfully saved!"
    elsif params[:message] == "audit"
      flash[:notice] = "Audit Logs PDF successfully saved!"
    end
    redirect_to cashadv_file_generator_index_path
  end
